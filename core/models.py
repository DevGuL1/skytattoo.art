...
class Language(models.Model):
    ...
    def load_defaults(cls):
        if not cls.objects.exists():
            cls.objects.create(code="ka", name="ქართული", is_default=True, is_active=True, order=0)
            cls.objects.create(code="en", name="English", is_default=False, is_active=True, order=1)
